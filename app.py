...
def getDB():
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # Directory where your Flask app is located
...
def groups():
    ...
    data = getData()
    print(data)
    data_for_json = {itm[0]: itm[1:] for itm in data}
    print(data_for_json)
    csv_file_path = 'website_data.csv'
    with open(csv_file_path, 'w') as csv_file:
        csv_file.write('user,source,destination,isDriver,numSeats\n')
        for key, value in data_for_json.items():
            csv_file.write(f"{key},{value[0]},{value[1]},{value[2]},{value[3]}\n")
    ...
    return render_template('one.html', img="output.png", txt=txt_content)
...
def add_entry():
    ...
    num_available_seats = data.get('num_seats')
    
    is_driver = int(is_driver)
    num_available_seats = int(num_available_seats)

    # Insert data into the database
    print(user, source, destination, is_driver, num_available_seats)
    db = getDB()
    db.execute('INSERT INTO addresses (user, source, destination, isDriver, numAvailableSeats) VALUES (?, ?, ?, ?, ?)',
               (user, source, destination, is_driver, num_available_seats))
    db.commit()
    # showDB()
...
def showDB():
    print("showDB")
    entries = getData()
    entriesDict = {entry[0]: entry[1:] for entry in entries}
    print(type(entriesDict))
    print(entriesDict)
    print("\n\n\n")
    return render_template('showDB.html', entries=entriesDict)
